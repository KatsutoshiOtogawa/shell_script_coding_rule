...
9. 変数の値が0以外の数字、空、未定義の場合0を返し、それ以外の値の場合は値をそのまま返す。
...
さらに言うと、
set -eだったら、b,cの書き方はプログラミング言語でいう!
だし、set使ってなかったら、?と似た感覚の処理になる同じ表記でもプログラミング言語で言う、!か?かが変わる。
っていうことです。

前の現場と今回の現場で同じ表記でも動き違うよね。
というのがありえるという話です。

これは直感的な動きでは無いし、できるだけ多くのエンジニア
が読める状態を作りたいという狙いに反するので私としては推奨しません。
